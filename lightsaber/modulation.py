"""The modulation matrix — Lightsaber's shared nervous system.

Sources (LFOs, ADSR envelopes, externally-driven values, the transport clock)
are sampled once per tick into a flat dict of named values. Routes then add
scaled source values onto named destination parameters, so any source can
drive any visual parameter without the generator that owns it knowing where
the value came from.

Destination keys are namespaced strings, e.g. "visual.speed", "scene.morph".
A generator simply calls `matrix.value("visual.speed", base)` to read its
modulated parameter.
"""
...
class Value(Source):
    """A plain externally-driven value: some other part of the engine writes
    `.current`, the matrix reads it."""
...
class ModMatrix:
    def __init__(self):
        ...
        # influence up or down (e.g. all routes driven by one LFO at once)
