"""3D for the vector generators — a camera and a projection pipeline, not a
game engine.

Keeps the world as 3D polylines, flies a slow camera through it, projects to
the same normalized 2D `Path`s everything downstream already speaks. Because
a scene only draws a few hundred strokes per frame (a browser-repaint
budget, see composite.py), the far plane does double duty: it culls distant
geometry (performance) and *is* the fog.

Depth cueing
------------
Depth is shown with COLOUR, configurable via the scene's `camera.depth`:

    mode = "hue"  : lerp near_color -> far_color by distance
    mode = "cull" : hard-drop anything past `far` (pure depth culling)
    mode = "both" : hue AND cull

Near-plane clipping is mandatory: segments crossing behind the camera are
cut, otherwise the perspective divide blows up.
"""
...
class DepthCue:
    ...
    def color_for(self, depth: float, near: float, far: float):
        ...
        return tuple(a + (b - a) * t for a, b in zip(self.near_color, self.far_color))
...
class Camera:
    ...
    def __init__(self, *, fov=60.0, near=0.4, far=14.0, speed=0.6,
        ...
        if waypoints and len(waypoints) >= 3:
            ...
            if self.mode == "path":
                ...
                print("[lightsaber] camera mode 'path' needs >=3 waypoints "
    ...
    def update(self, t: float, dt: float, matrix=None):
        ...
        if self.mode == "fly":
            ...
            # means modulation changes the rate of travel, never the place.
            #
            ...
            # whenever modulation drove the speed to zero, which is exactly
            # when a held, still frame most wants to keep breathing.
            ...
            # whatever is directly in front, while a lookahead of roughly a
            # third of a lap aims across the space and keeps the scene's bulk
            # in the cone. Following the curve also means the view leads into
            # turns instead of swinging wide of them.
            ...
            # routes like lfo_slow -> camera.speed change `speed` every
...
def make_camera(spec: dict) -> Camera:
    ...
    return Camera(
        ...
        # None (the default) means "a tenth of the lap" — see Camera.__init__
