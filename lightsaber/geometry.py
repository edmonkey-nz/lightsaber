...
[0, 1] — the natural unit for the vector generators throughout Lightsaber,
warped per-shape into a canvas polygon's quad by the browser (see
composite.py, index.html, output.html).
"""
...
def clamp_frame(frame: Frame) -> Frame:
    """Clamp all coordinates into [-1, 1] — the homography warp (see
    composite.py/index.html/output.html) assumes source content stays in
    this range."""
    for p in frame:
