...
to Claude. Keeps Lightsaber fully usable offline: the API is an *optional*
...
def _forest_3d(keyword: str) -> SceneSpec:
    return SceneSpec(
        ...
        # depth shown with COLOUR (near green -> far deep-blue). Switch mode
        # to "cull" for hard depth culling instead.
        camera=dict(fov=62, near=0.4, far=14.0, speed=0.6, height=1.0,
                    max_strokes=90,
                    depth=dict(mode="hue", near_color=[0.3, 0.95, 0.5],
                               far_color=[0.08, 0.15, 0.5])),
        modulation=[
            # a slow LFO nudges how fast you drift through the trees
            dict(source="lfo_slow", dest="camera.speed", depth=0.5, bias=0.0),
        ],
    )


def _world(name, nodes, camera, palette):
    return SceneSpec(
        ...
        camera=camera,
        modulation=[
            dict(source="lfo_slow", dest="camera.speed", depth=0.6),
        ],
    )
...
def _studio_world(keyword: str) -> SceneSpec:
    ...
    return SceneSpec(
        ...
        camera=camera,
        modulation=[
            dict(source="lfo_slow", dest="camera.speed", depth=0.5),
        ],
    )


def local_scene(keyword: str, aspect: str = "1:1") -> SceneSpec:
    spec = _local_scene(keyword)
    spec.aspect = aspect if aspect in ("1:1", "16:9", "4:3") else "1:1"
    return spec


def _local_scene(keyword: str) -> SceneSpec:
    kw = keyword.lower()
    ...
    return SceneSpec(
        ...
        palette=pal,
        modulation=[
            # calm defaults: a slow LFO breathes the visuals, a mid LFO
            # nudges turbulence
            dict(source="lfo_slow", dest="visual.speed", depth=0.05, bias=0.0),
            dict(source="lfo_mid", dest="visual.turbulence", depth=0.4),
        ],
    )
